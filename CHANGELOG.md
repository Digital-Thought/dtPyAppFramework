...
- Bump cryptography library from 44.0.0 to 46.0.5 to address elliptic curve
  public key validation vulnerability (SECT curves). The vulnerability allowed
  attackers to provide public key points from small-order subgroups, potentially
  leaking private key bits via ECDH or enabling ECDSA signature forgery.

## [4.4.0] - 2026-03-07

### Added

- Native system temp path option via `USE_SYSTEM_TEMP=TRUE` environment variable
  - When enabled, uses the OS temp directory directly without app-specific subdirectory
...
- Add comprehensive configuration reference documentation (`config-reference.rst`)

## [4.3.0] - 2026-02-16

### Changed

- **Breaking:** The all-users keystore (`App_Local_Store`) is no longer created
  by default. Set environment variable `ALL_USER_KS=TRUE` to enable it.
  This reduces permission errors and simplifies default deployments where
  only per-user secrets are needed.

## [4.2.1] - 2026-02-01

### Fixed

- Fix flaky dictionary attack resistance test that false-positived on short
  substrings (e.g. "key") appearing by chance in random 24-character secrets
